#' @title precision simulation with multi-classification
#'
#' @description  Multi-Classification analysis applied on different normalization and study design.
...
#' \code{precision.simulate.multiclass} requires the training and test sets for both estimated biological effects and estimated handling effects.
...
#'
#' Fourth, choose classfication methods, specified with  \code{class.list}. This will fit the chosen classification methods on the training
...
#' User can provide a list of normalization methods given the functions are supplied (also see norm.funcs).
#' @param class.list  list of strings for classification methods to be compared in the simulation study.
...
#'
#' @import mclust
#' @export precision.simulate.multiclass
#'
#' @examples
#' \dontrun{
#' set.seed(101)
...
"precision.simulate.multiclass" <- function(seed, N,
  ...
  stopifnot(all(class.list %in% c("PAM", "LASSO", "ClaNC", "ranFor", "SVM", "kNN", "DLDA", "custom")))
  ...
  for (k in 1:N) {# each of the N simulation
    ...
    class.all = c("PAM", "LASSO", "ClaNC", "ranFor", "SVM", "kNN", "DLDA", "custom")
    new.class.funcs <- switch.classifier.funcs.class(class.list = class.all,
                                                     class.funcs = class.funcs,
                                                     pred.funcs = pred.funcs)

    # Do classification

    for (class.met in class.list){
      cat(paste0("- build ", class.met, " (Train: ", train.design.met, " & ", train.norm.met, "; Test: ", test.design.met, " & ", test.norm.met, ")\n"))
      class.func <- new.class.funcs$build.funcs[class.all == class.met]
      pred.func <- new.class.funcs$pred.funcs[class.all == class.met]
      ...
      # store model
      eval(parse(text = paste0("model_store[['", class.met, "']][", k, "] <- list(", train.design.met2, ".",
      ...
      # store errors
      eval(parse(text = paste0("error_store[['", class.met, "']][['internal']][", k, "]<- ", train.design.met2, ".",
                               tolower(class.met), ".int.", train.norm.met2, "$mc")))
      eval(parse(text = paste0("error_store[['", class.met, "']][['external']][", k, "] <- ", test.design.met2, ".",
                               tolower(class.met), ".pred.", test.norm.met2, "$mc")))

      # store adjusted random index
      eval(parse(text = paste0("ari_store[['", class.met, "']][", k, "]<- adjustedRandIndex(group.id.te, ",
